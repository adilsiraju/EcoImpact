...
class ImpactCalculator:
    ...
    def predict_impact(self, investment_amount, category_names, project_duration_months=12, project_scale=1,
                       location='North India', technology_type=None):
        ...
        X = np.array([[investment_amount] + category_vector + [project_duration_months, project_scale, location_encoded,
                                                               technology_encoded]])
        X_transformed = X.copy()
        X_transformed[:, 0] = np.log1p(X[:, 0])
...
def main():
    ...
    carbon, energy, water = calc.predict_impact(investment, [category], technology_type=tech)
